...
Вход идёт через сервис `caddy` (порт 80/443); сам `api` наружу не публикуется. Для HTTPS
задайте `SITE_ADDRESS=ваш-домен` в `.env` — Caddy сам выпустит TLS-сертификат.
...
Проверка живости API: `GET http://localhost/healthz` → `{"ok": true}`.
...
Несколько инстансов `api` уже работают из коробки: `api` объявлен через `expose` (без
фиксированного хост-порта), а сервис `caddy` балансирует по репликам — просто
`--scale api=3`. Подробнее — [`docs/12-scaling.md`](docs/12-scaling.md).

## 7. Перед публичным продом (кратко)

- HTTPS-прокси перед `api`; `CORS_ALLOW_ORIGINS` сузить до вашего домена.
- Порты `5432`/`6379` наружу уже НЕ публикуются (только сеть compose, C1); задайте сильный
  `POSTGRES_PASSWORD` (+ синхронно `POSTGRES_DSN`) и пароль на Redis при внешнем доступе.
