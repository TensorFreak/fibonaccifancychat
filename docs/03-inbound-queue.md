...
- **Reaper зависших задач** (`XAUTOCLAIM`), **dead-letter** отравленных и уникальный `CONSUMER_NAME` — см. [10. Харденинг](10-hardening.md). Дедлеттер входящего сообщения **сдвигает гейт порядка** (`applied` до `seq`), иначе преемник завис бы на `order_gap_timeout_ms` в ожидании уже выброшенного хода (H3-review).
