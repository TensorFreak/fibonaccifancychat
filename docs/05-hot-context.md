...
> ⚠️ **Регидратация срабатывает только на ПУСТОМ окне.** Отсюда жёсткое требование к воркеру: `load_context` для регидратации должен вызываться **до** `append_context` текущего хода (см. Шаг 0 в [04](04-llm-worker.md)). Иначе `append_context` первым положит новое сообщение в протухшее окно, `load_context` увидит непустой список и историю из Postgres НЕ поднимет — контекст диалога потеряется после простоя. Закрыто тестом `tests/test_context_rehydration.py`.
