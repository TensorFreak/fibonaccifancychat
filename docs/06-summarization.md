...
> **Надёжность заголовка (reasoning-модели).** Гибридные модели (напр. `deepseek-v4`) при малом `title_max_tokens` тратят бюджет на «размышление» и отдают пустой видимый `content`, либо встраивают его в content как `<think>…</think>`. Поэтому `generate_title`: (1) `title_max_tokens=100` — даёт reasoning дойти до заголовка; (2) вырезает `<think>…</think>` (и висящий незакрытый блок) перед извлечением названия; (3) если заголовок всё же пуст — **логирует сырой ответ модели** и берёт осмысленное имя из первого сообщения пользователя. Записать буквальную заглушку «Новый чат» нельзя: она непустая, поэтому в UI неотличима от «без названия», но навсегда блокирует перегенерацию (`set` пишет лишь `WHERE title IS NULL`) и повторную постановку задачи (NX-маркер живёт сутки) — чат застревает. Хотите «красивые» LLM-заголовки на reasoning-модели, которая при малом бюджете отдаёт пустой `content` (в логах `empty title … raw LLM output=''`) — генерируйте заголовок отдельной моделью: `TITLE_LLM_MODEL=<дешёвая не-reasoning>` (пусто = основная `LLM_MODEL`). `generate_title` прокидывает её в `complete(..., model=…)`. Как быстрый (но менее надёжный/медленный) обходной путь — поднять `TITLE_MAX_TOKENS`. Регрессии — в `tests/test_generate_title.py`.
