...
|---|---|---|
| `context_window_tokens` | 128000 | полное окно модели — **реальный потолок** (см. ниже); дефолт согласован с `llm_model=gpt-4o-mini` |
...
- `prompt_token_budget <= 0` → **авто**: `prompt_token_budget = ceiling` (использовать окно
  по максимуму — для больших моделей вроде 1M);
...
окно 128k, ответ 4096, запас 5% → авто-бюджет **117 504**; заданные 300000 → зажаты до
117 504; окно 8k, ответ 1024 → бюджет 6 576.

> **⚠️ Согласованность окна и модели (H1-review).** `context_window_tokens` в режиме
> «авто» задаёт почти весь бюджет промпта, поэтому значение БОЛЬШЕ реального окна модели —
> это не «запас», а гарантированная поломка: промпт соберётся под окно, которого у модели
> нет, и каждый длинный диалог упрётся в `context_length_exceeded`. Раньше дефолт был
> `256000` при дефолтной `llm_model=gpt-4o-mini` (реальное окно 128k) — прямой рассинхрон.
> Теперь дефолт `128000`, а валидатор `_guard_context_window_matches_model` **фейлит старт**,
> если `context_window_tokens` превышает известное окно заданной модели (сверка по префиксу
> имени; неизвестная модель — свой прокси/локальная — проверку пропускает). Меняете
> `LLM_MODEL` — синхронно правьте `CONTEXT_WINDOW_TOKENS`.
...
- `context_window_tokens` — из ТТХ модели (валидатор не даст задать больше известного окна);
