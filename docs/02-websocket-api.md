...
0. **Авторизация:** проверить токен и владение диалогом (см. [10](10-hardening.md), C1).
   Перед обращением к БД — потолок одновременных ws-соединений на пользователя
   (`ws_max_connections_per_user`, H2): при превышении сокет закрывается кодом `1013`
   (учёт — ZSET дедлайнов в Redis, общий на все инстансы; живое соединение продлевает свою
   запись heartbeat'ом, запись мёртвого инстанса выпадает по TTL — не уходит в минус, R7-2).
