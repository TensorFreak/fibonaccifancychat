...
class Settings(BaseSettings):
    ...
    # Рейтлимит ЧАСТОТЫ подключений ws per-user (#2). Лимит concurrent-соединений выше НЕ
    # покрывает цикл connect/disconnect на новые UUID: каждый коннект — транзакция в
    # Postgres (ensure_conversation создаёт строку диалога), т.е. вектор DoS на БД. Режем
    # частоту в фиксированном окне (INCR+EXPIRE, общий на все инстансы). Щедро для человека
    # (несколько вкладок/реконнектов), но отсекает флуд коннектов одним токеном.
    ws_connect_rate_max: int = 60        # подключений за окно на пользователя
    ws_connect_rate_window_seconds: int = 60

    # Рейтлимит попыток register/login per-IP (#3). Без него: (а) брутфорс пароля не
    # ограничен; (б) каждая попытка гоняет bcrypt (~100мс CPU) — флуд эндпоинтов это
    # CPU-DoS на api. Фиксированное окно per-IP (X-Forwarded-For от доверенного прокси).
    auth_rate_max: int = 10              # попыток за окно с одного IP
