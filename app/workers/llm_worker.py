...
async def process(r, fields: dict):
    ...
    try:
        ...
        # 0) РЕГИДРАТАЦИЯ холодного окна ДО записи текущего хода (иначе потеря контекста
        #    после простоя). load_context тянет историю из Postgres ТОЛЬКО когда окно пусто.
        #    Если сделать это ПОСЛЕ append_context (шаг 1), тот положит текущее сообщение в
        #    протухшее окно, окно станет непустым, и регидратация НЕ сработает — в промпт
        #    уйдёт лишь текущее сообщение, а вся история (хотя она цела в Postgres) выпадет.
        #    Диалог «забывал всё» после ctx_ttl простоя (или моргания/эвикции Redis). Тёплое
        #    окно этот вызов проходит мгновенно (lrange непустой, в PG не идём). Текущего
        #    сообщения в окне ещё нет (вставляем его ниже), поэтому append даст эхо как обычно.
        await load_context(r, conversation_id)
